...
def locate_env_file() -> Path | None:
    """Find the BridgeAgent .env file.

    Lookup order (first match wins):
      1. $BRIDGEAGENT_ENV_FILE
      2. ./.env (developer / repo-clone flow)
      3. ~/.config/bridgeagent/.env (standard user install)
      4. ~/.bridgeagent/.env (Windows / legacy fallback)
    """
    override = os.environ.get("BRIDGEAGENT_ENV_FILE")
    if override:
    ...
    candidates = [
        Path.cwd() / ".env",
        Path.home() / ".config" / "bridgeagent" / ".env",
        Path.home() / ".bridgeagent" / ".env",
    ]
    for p in candidates:
        if p.is_file():
            return p
...
def _safe_load_dotenv(path: Path) -> None:
    ...
    try:
        ...
        logger.warning(
            ...
            "`bridgeagent setup` to regenerate it.", path,
        )
        return
...
# Agent wallet: a trade-only, revocable Hyperliquid agent key minted by the
# Byreal Perps CLI bootstrap flow (Privy-signs an `approveAgent` against
# Hyperliquid). It cannot withdraw funds — that permission is exchange-
# enforced. The Byreal CLI stores the key in ~/.byreal-perps/perps.db; the
# bridgeagent wizard reads it from there and writes it here.
BRIDGE_AGENT_PRIVATE_KEY = os.getenv("BRIDGE_AGENT_PRIVATE_KEY", "")
BRIDGE_MAIN_ADDRESS = os.getenv("BRIDGE_MAIN_ADDRESS", "")
