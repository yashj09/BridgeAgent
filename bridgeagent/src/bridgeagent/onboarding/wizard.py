...
Design notes:
  ...
  * `bridgeagent setup` re-runs the wizard with existing values as defaults.
"""
...
from bridgeagent.onboarding.validators import (
    PairingResult,
    is_valid_address,
    is_valid_private_key,
    verify_agent_pairing,
)
from bridgeagent.onboarding.logo import print_logo
...
def default_config_path() -> Path:
    return Path.home() / ".config" / "bridgeagent" / ".env"
...
def run_wizard(existing: Optional[Dict[str, str]] = None) -> Dict[str, str]:
    ...
    `existing` is the current `.env` as a dict (for `bridgeagent setup`);
    ...
    print(
        "  BridgeAgent signs trades with an " + _bold("agent wallet") + " — a\n"
    ...
    while True:
        main_addr = _prompt(
            ...
            default=existing.get("BRIDGE_MAIN_ADDRESS") or None,
        )
    ...
    while True:
        try:
            ...
            print()
            raise
        if not agent_key and existing.get("BRIDGE_AGENT_PRIVATE_KEY"):
            agent_key = existing["BRIDGE_AGENT_PRIVATE_KEY"]
    ...
    if _yes_no(
        ...
        while True:
            ...
            if raw.lower() == "skip":
                print(_dim("  skipped — AI disabled, run `bridgeagent setup` later to add"))
                skipped = True
                break
            if raw:
    ...
    cfg["BRIDGE_NETWORK"] = network
    cfg["BRIDGE_MAIN_ADDRESS"] = main_addr
    cfg["BRIDGE_AGENT_PRIVATE_KEY"] = agent_key
    cfg["BRIDGE_RECONCILE_ON_BOOT"] = "1" if reconcile else "0"
    ...
    if not _yes_no("Save and launch?", default=True):
        print(_yellow("Discarded. Run `bridgeagent` again to restart the wizard."))
        raise KeyboardInterrupt
...
def save_config(cfg: Dict[str, str], path: Optional[Path] = None) -> Path:
    ...
    order = [
        "BRIDGE_NETWORK",
        "BRIDGE_MAIN_ADDRESS",
        "BRIDGE_AGENT_PRIVATE_KEY",
        "BRIDGE_RECONCILE_ON_BOOT",
        "AWS_ACCESS_KEY_ID",
    ...
    lines = [
        "# BridgeAgent config - generated by `bridgeagent setup`",
...
# ---------------------------------------------------------------------------
# Helper: parse an existing .env into a dict (for `bridgeagent setup`)
# ---------------------------------------------------------------------------
