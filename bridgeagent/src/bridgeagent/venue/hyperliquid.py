"""
Hyperliquid venue implementation for BridgeAgent.

This is the only concrete `Venue` today. It wraps the official
hyperliquid-python-sdk and exposes the methods declared in
`bridgeagent.venue.base.Venue`.

The same agent wallet works for Byreal Perps because Byreal Perps is a
Privy-bridged frontend over Hyperliquid: the Byreal CLI's `account init`
mints a vanilla Hyperliquid agent key via `approveAgent` and stores it
locally. After bootstrap, this venue talks to the Hyperliquid API
directly with no Byreal layer at runtime.

Maintains two SDK connections:
  - Mainnet Info client (read-only, no key needed) for market data
  - Testnet/Mainnet Exchange client (needs private key) for trading
...
class HyperliquidVenue:
    """Async Hyperliquid implementation of the `Venue` protocol."""
    ...
    def __init__(self, private_key: str = "", testnet: bool = True):
        ...
        key = private_key or config.BRIDGE_AGENT_PRIVATE_KEY
        if key:
            account = Account.from_key(key)
            agent_addr = account.address
            # Agent-wallet mode: agent key signs, trades target the main
            # wallet via account_address. If BRIDGE_MAIN_ADDRESS is absent we
            ...
            main_addr = config.BRIDGE_MAIN_ADDRESS or agent_addr
            ...
            mode = "agent->main" if config.BRIDGE_MAIN_ADDRESS else "single-key"
            logger.info(
    ...
    async def get_candles_window(
        ...
        """Fetch OHLCV candles for *coin* over an explicit ms window."""
        candles = await asyncio.to_thread(
    ...
    async def get_l2_book(self, coin: str) -> Dict[str, Any]:
        ...
        strategy. Returns {} on rate-limit so the caller doesn't crash."""
        if self._is_rate_limited():
            return {}
        try:
            return await asyncio.to_thread(self.info.l2_snapshot, coin)
        except Exception as exc:
            if self._is_429(exc):
                self._note_rate_limit(60.0)
                return {}
    ...
    async def get_open_orders(self) -> List[Dict[str, Any]]:
        """Open orders for the configured `address`. Returns [] if no
        address is configured or if rate-limited."""
        if not self.address:
            return []
        if self._is_rate_limited():
            return []
