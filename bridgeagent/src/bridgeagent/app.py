...
from bridgeagent.core.state import AgentState, Signal, ActivePosition
from bridgeagent.core.client import HyperLiquidClient
from bridgeagent.core.risk import RiskManager
from bridgeagent.core.regime import RegimeDetector
from bridgeagent.core.candle_cache import CandleCache
from bridgeagent.core.hypedexer_client import HypeDexerClient
from bridgeagent.core.liquidation_aggregator import LiquidationAggregator
...
from bridgeagent.strategies.ai_wrapper import AIWrapper
from bridgeagent.tui.screens.dashboard import DashboardScreen
from bridgeagent.tui.screens.strategy_config import StrategyConfigScreen
from bridgeagent.tui.screens.trade_journal import TradeJournalScreen
from bridgeagent.tui.screens.analytics import AnalyticsScreen
from bridgeagent.tui.screens.confirm_kill_modal import ConfirmKillModal
from bridgeagent.tui.screens.reconcile_modal import ReconcileModal
from bridgeagent.tui.tuning_advice import suggest as _suggest_params, probe_alternatives

logger = logging.getLogger(__name__)


class BridgeAgentApp(App):
    ...
    def __init__(self):
        ...
        self.state.add_log("[INIT] BridgeAgent v2 starting up...")
    ...
    def on_mount(self):
        ...
        # one-shot — we clear it from ~/.config/bridgeagent/.env below once
        # the reconcile worker finishes, so the next boot stays quiet.
        if config.BRIDGE_RECONCILE_ON_BOOT:
                ...
                if self.state.ai_enabled:
                    ...
                    if err and err != self._last_ai_error_logged:
                        self.state.add_log(
                            ...
                            f"check AWS creds (run `bridgeagent setup`)"
                        )
    ...
    def _clear_reconcile_flag(self) -> None:
        """Flip BRIDGE_RECONCILE_ON_BOOT=0 in the user's .env after one fire.
        ...
        try:
            ...
            found = False
            for line in lines:
                if line.startswith("BRIDGE_RECONCILE_ON_BOOT="):
                    new_lines.append("BRIDGE_RECONCILE_ON_BOOT=0")
                    found = True
                else:
            ...
            try:
                ...
                if self.hypedexer.auth_failed and not auth_alerted:
                    self.state.add_log(
                        ...
                        "running without fresh data. Run `bridgeagent setup`."
                    )
                    auth_alerted = True
                ...
                # for free-tier users running BridgeAgent continuously.
...
def _config_is_complete() -> bool:
    ...
    We treat "no agent key" as "run the wizard". BRIDGE_MAIN_ADDRESS is
    ...
    """
    from bridgeagent.onboarding.validators import is_valid_private_key
    return bool(config.BRIDGE_AGENT_PRIVATE_KEY) and is_valid_private_key(
        config.BRIDGE_AGENT_PRIVATE_KEY
    )


def _run_setup_flow(force: bool = False) -> None:
    """Launch the CLI wizard. Writes ~/.config/bridgeagent/.env on success.

    force=True → always run (used by `bridgeagent setup`), pre-filling with
    existing values.
    force=False → run only when config is incomplete.
    """
    from bridgeagent.onboarding.wizard import (
...
def main() -> None:
    """Console-script entry point (`bridgeagent` command)."""
    import sys
    ...
    if len(sys.argv) > 1 and sys.argv[1] == "setup":
        ...
        # the user doesn't have to type `bridgeagent` a second time.
    else:
    ...
    if not _config_is_complete():
        ...
        if not config.BRIDGE_AGENT_PRIVATE_KEY:
            print("No agent wallet configured. Run `bridgeagent setup` to set one up.")
        else:
            print(
                "BRIDGE_AGENT_PRIVATE_KEY is malformed (expected 0x + 64 hex chars). "
                "Run `bridgeagent setup` to regenerate."
            )
        raise SystemExit(1)
